...
export const dictionaryVectorLinearCombination =
  monomial_basisjs.dictionaryVectorLinearCombination;
...
export class Sseq extends EventEmitter {
  ...
  getDrawnElements(page, xmin, xmax, ymin, ymax) {
    ...
    let display_edges = this.edges.filter(
      (e) =>
        ...
        Sseq._drawClassOnPageQ(e.source, page) &&
        Sseq._drawClassOnPageQ(e.target, page) &&
  ...
  static fromJSONObject(json) {
    ...
    else serializeSseqFields = Sseq.serializeSseqFields;
  ...
  toJSON() {
    ...
    let node_map = new StringifyingMap((n) =>
      JSON.stringify(Sseq._serializeNode(n)),
    );
