...
import { renderLatex, ensureMath } from "./Interface";
...
export class EditorDisplay extends SidebarDisplay {
  constructor(container, sseq) {
    ...
    this.sidebar.footer.addButton("Download SVG", () =>
      this.downloadSVG("sseq.svg"),
    );
    this.sidebar.footer.addButton("Save", () =>
    ...
    this.title_edit_link.addEventListener("click", () => {
      ...
      if (this.title_edit_link.innerHTML == "OK") {
        ...
        this.sseq.undo.addValueChange(c, "name", old_name, c.name, () =>
    ...
    this.nodeTab.on("show", () => {
      ...
      let c = this.selected.c;
      if (c.name) {
        this.title_text.innerHTML =
          renderLaTeX(ensureMath(c.name)) + ` - (${c.x}, ${c.y})`;
      } else {
    ...
    // Differentials tab
    this.differentialTab = new Panel.DifferentialPanel(
    ...
    // Structline tab
    this.structlineTab = new Panel.StructlinePanel(
      this.classPanel.container,
      this,
    );
    ...
    switch (this.state) {
      ...
      case STATE_RM_DIFFERENTIAL:
        ...
        for (let e of s.edges)
          if (e.type === "Differential" && e.target == t) sseq.deleteEdge(e);
        this.sseq.undo.addMutationsToUndoStack();
        this.sidebar.showPanel();
        break;
      ...
      case STATE_RM_STRUCTLINE:
        this.sseq.undo.startMutationTracking();
        for (let e of s.edges)
          if (e.type === "Structline" && e.target == t) sseq.deleteEdge(e);
        this.sseq.undo.addMutationsToUndoStack();
        this.sidebar.showPanel();
        break;
      case STATE_RM_EDGE:
        this.sseq.undo.startMutationTracking();
        for (let e of s.edges) if (e.target == t) sseq.deleteEdge(e);
        this.sseq.undo.addMutationsToUndoStack();
        this.sidebar.showPanel();
        break;
      default:
