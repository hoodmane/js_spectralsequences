...
export class Panel extends EventEmitter {
  ...
  addButton(text, callback, extra = {}) {
    ...
    if (extra.shortcuts)
      for (let k of extra.shortcuts) Mousetrap.bind(k, callback);
  ...
  addLinkedInput(label, target, type, mementoObject) {
    ...
    i.addEventListener("change", (e) => {
      let target_pre;
      if (mementoObject) {
        mementoObject = Panel.unwrapProperty(
          this.display,
          mementoObject.split("."),
        );
      ...
      if (this.display.sseq.undo) {
        if (mementoObject) {
          ...
          this.display.sseq.undo.addValueChange(t, prop, old_val, new_val, () =>
...
export class DifferentialPanel extends Panel {
  constructor(parentContainer, display) {
    ...
    this.on("show", () => {
      ...
      let edges = this.display.selected.c.edges
        .filter((e) => e.type === "Differential")
