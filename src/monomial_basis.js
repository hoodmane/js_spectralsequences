...
export function range(start, stop, step = 1) {
  ...
  return Array(Math.ceil((stop - start + step) / step))
    .fill(start)
    .map((x, y) => x + y * step);
}
...
export function dictionaryVectorLinearCombination(
  vector_list,
  coefficient_list,
) {
  let scaled_list = [];
...
class monomial_ring {
  ...
  getElementFromVector(exponent_vector, module_generator = "") {
    if (
      module_generator !== "" &&
      !this._module_generators_dict.hasOwnProperty(module_generator)
    ) {
...
class monomial_element {
  ...
  _initializeDegree() {
    ...
    if (this._module_generator !== "") {
      stem_degree +=
        this._ring._module_generators_dict[this._module_generator][0];
      filtration +=
        this._ring._module_generators_dict[this._module_generator][1];
    }

    for (let i = 0; i < this.exponent_vector.length; i++) {
      stem_degree +=
        this._ring._var_degree_list[i][0] * this.exponent_vector[i];
...
export class monomial_basis {
  ...
  addModuleGenerator(gen_name, bidegree, callback) {
    ...
    for (let i = 0; i < l.length; i++) {
      ...
      if (this.sseq.xRange && this.sseq.drop_out_of_range_classes) {
        ...
        if (
          stem < this.sseq.xRange[0] - 10 ||
          stem > this.sseq.xRange[1] + 10
        ) {
          console.log("dropped");
          continue;
        }
      ...
      if (this.sseq.yRange && this.sseq.drop_out_of_range_classes) {
        if (
          stem < this.sseq.yRange[0] - 10 ||
          stem > this.sseq.yRange[1] + 10
        ) {
          continue;
        }
      }
      ...
      if (this.sseq.yRange && this.sseq.max_differential_length) {
        if (
          filtration >
            this.sseq.yRange[1] + this.sseq.max_differential_length ||
          filtration < this.sseq.yRange[0]
        ) {
          continue;
        }
      }
  ...
  static _validateVarSpec(var_spec, var_degree_dict, index) {
    ...
    if (var_spec.length < 2 || var_spec.length > 4) {
      throw (
        err_string +
        " Variable specification should be of length at least 2 and at most 4."
      );
    }
    ...
    if (typeof var_spec[0] !== "string") {
      throw (
        err_string +
        " First entry of variable specification should be a string, the name of a variable."
      );
    }
    ...
    if (!var_degree_dict.hasOwnProperty(var_spec[0])) {
      throw (
        ...
        " Variable '" +
        var_spec[0] +
        "' does not have an entry in the degree dictionary"
      );
    }
...
export class slice_basis {
  ...
  addDifferentialLeibniz(
    ...
    for (let exponent_vector of product(
      ...
      if (
        !this.has(cur_source_slice) ||
        !this.get(cur_source_slice).has(stem)
      ) {
        continue;
      }
      ...
      if (
        this.has(cur_target_slice) &&
        this.get(cur_target_slice).has(stem - 1)
      ) {
