...
export class Display extends EventEmitter {
  ...
  _updateScale() {
    ...
    if (this.sseq.xRange) {
      if (
        ...
        xScale(this.sseq.xRange[0] - this.domainOffset) > this.leftMargin
      ) {
        this.zoom.translateBy(
          zoomD3Element,
          (this.leftMargin - xScale(this.sseq.xRange[0] - this.domainOffset)) /
            scale,
          0,
        );
      } else if (
        xScale(this.sseq.xRange[1] + this.domainOffset) < this.clipWidth
      ) {
        this.zoom.translateBy(
          zoomD3Element,
          (this.clipWidth - xScale(this.sseq.xRange[1] + this.domainOffset)) /
            scale,
          0,
        );
      }
    ...
    if (this.sseq.yRange) {
      if (
        ...
        yScale(this.sseq.yRange[0] - this.domainOffset) < this.clipHeight
      ) {
        this.zoom.translateBy(
          zoomD3Element,
          0,
          (this.clipHeight - yScale(this.sseq.yRange[0] - this.domainOffset)) /
            scale,
        );
      } else if (
        yScale(this.sseq.yRange[1] + this.domainOffset) > this.topMargin
      ) {
        this.zoom.translateBy(
          zoomD3Element,
          0,
          this.topMargin -
            yScale(this.sseq.yRange[1] + this.domainOffset) / scale,
        );
      }
    }
  ...
  _updateGridAndTickStep() {
    ...
    this.xGridStep =
      Math.floor(this.xTickStep / 5) === 0 ? 1 : Math.floor(this.xTickStep / 5);
    this.yGridStep =
      Math.floor(this.yTickStep / 5) === 0 ? 1 : Math.floor(this.yTickStep / 5);
  ...
  _drawTicks(context) {
    ...
    for (
      let i = Math.floor(this.xTicks[0]);
      i <= this.xTicks[this.xTicks.length - 1];
      i += this.xTickStep
    ) {
    ...
    for (
      let i = Math.floor(this.yTicks[0]);
      i <= this.yTicks[this.yTicks.length - 1];
      i += this.yTickStep
    ) {
  ...
  _drawGridWithOffset(context, xoffset, yoffset) {
    ...
    for (
      let col =
        Math.floor(this.xmin / this.xGridStep) * this.xGridStep - xoffset;
      col <= this.xmax;
      col += this.xGridStep
    ) {
    ...
    for (
      let row =
        Math.floor(this.ymin / this.yGridStep) * this.yGridStep - yoffset;
      row <= this.ymax;
      row += this.yGridStep
    ) {
  ...
  _drawEdges(context, edges) {
    for (let e of edges) {
      if (!e || e.invalid || !e.visible) {
        // TODO: should probably log some of the cases where we skip an edge...
        continue;
      }
      ...
      if (
        !e.sourceOffset ||
        (e.sourceOffset.x === 0 && e.sourceOffset.y === 0)
      ) {
      ...
      if (e.bend) {
        ...
        let control1X =
          sourceX + Math.cos(angle + bendAngle) * looseness * distance;
        let control1Y =
          sourceY + Math.sin(angle + bendAngle) * looseness * distance;
        let control2X =
          targetX - Math.cos(angle - bendAngle) * looseness * distance;
        let control2Y =
          targetY - Math.sin(angle - bendAngle) * looseness * distance;
  ...
  _onMousemove(e) {
    ...
    if (this.mouseover_node) {
      if (
        this.nodes.includes(this.mouseover_node) &&
        this.context.isPointInPath(this.mouseover_node.path, this.x, this.y)
      ) {
        return;
      } else {
    ...
    let node = this.nodes.find((n) =>
      this.context.isPointInPath(n.path, this.x, this.y),
    );
    if (node) {
      redraw = true;
      node.highlight = true;
  ...
  drawSVG(context, xml) {
    ...
    context.drawImage(
      ...
      (this.canvasWidth / (this.xmaxFloat - this.xminFloat)) *
        (this.sseq.xRange[1] - this.sseq.xRange[0] + 1),
      (this.canvasHeight / (this.ymaxFloat - this.yminFloat)) *
        (this.sseq.yRange[1] - this.sseq.yRange[0] + 1),
    );
  }
